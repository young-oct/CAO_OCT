...
class image:
    def __init__(self, img_ary, g_var=0.005, s_var=0.1):
        self.img_original = img_ary

        self.g_var = g_var
        self.s_var = s_var

        self.img_norm = self.normalize_image()
        self.img_noise = self.add_noise()

        self.entropy = self.image_entropy()

    def normalize_image(self):
        return abs(self.img_original) / np.max(abs(self.img_original))

    def add_noise(self):
        img_noise = random_noise(self.img_original, mode='gaussian', var=self.g_var)
        return random_noise(img_noise, mode='speckle', var=self.s_var)

    def image_entropy(self):
        temp_img = self.normalize_image()
        entropy = 0
...
class zernike:
    ...
    def zernike_plane(i: object = None, r: object = None, theta: object = None) -> object:
